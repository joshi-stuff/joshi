...
generate.stub = function (fnName, functions, throws, types) {
	...
	const args = fn.args;
	const inArgs = args.filter(function (arg) {
		return !arg.out || arg.in_out;
	});
	const outArgs = args.filter(function (arg) {
		return arg.out || arg.in_out;
	});
	...
	const lines = [].concat(
		'static duk_ret_t ' +
			generate.FN_NAME_PREFIX +
			FN +
			'(duk_context* ctx) {',
		generate.tabify(
			1,
			args.reduce(function (lines, arg, idx) {
				return lines.concat(generate.declare_variable(arg, types));
			}, [])
		),
		'',
		generate.tabify(
			...
			inArgs.reduce(function (lines, arg, idx) {
				return lines.concat(generate.pop_variable(arg, types, idx));
			}, [])
		),
		'',
		'	errno = 0;',
